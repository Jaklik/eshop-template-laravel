...
<div class="container mx-auto">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        @foreach ($products as $product)
            <div class="max-w-md w-full bg-white border-gray-900 shadow-lg rounded-xl p-6">
                <div class="flex flex-col ">
                <div class="">
                    <div class="relative h-62 w-full mb-3">
                    <div class="absolute flex flex-col top-0 right-0 p-3">
                        <button class="transition ease-in duration-300 bg-white border-gray- hover:text-purple-500 shadow hover:shadow-md text-gray-500 rounded-full w-8 h-8 text-center p-1"><svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                        </svg></button>
                    </div>
                    @if ($product->thumbnail_image == null)
                    <img src="{{ asset('storage/products/cover_image.jpeg') }}" alt="Just a flower" class=" w-full   object-fill  rounded-2xl">
                    @else
                    <img src="{{ asset('storage/'.$product->thumbnail_image) }}" alt="Just a flower" class=" w-full   object-fill  rounded-2xl">
                    @endif
                    </div>
                    <div class="flex-auto justify-evenly">
                    <div class="flex flex-wrap ">
                        <div class="flex items-center w-full justify-between min-w-0 ">
                        <h2 class="text-lg mr-auto cursor-pointer text-black hover:text-purple-500 truncate ">{{ $product->name }}</h2>
                        <div class="flex items-center uppercase bg-green-400 text-white text-xs px-2 py-1 ml-3 rounded-lg">
                            skladem</div>
                        </div>
                    </div>
                    <div class="text-xl text-black font-semibold mt-1">{{ $product->price }} Kč</div>
